  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        <SVGs beams={beams} width={width} height={height} baseColor={baseColor} accentColor={accentColor} gradientColors={gradientColors} />
      </div>
    </div>
  );
};
      ...
      {beams.map((beam: BeamPath, index: number) => (
        <g key={index}>
          ...
          {beam.connectionPoints?.map((point, pointIndex) => (
            <circle key={pointIndex} cx={point.cx} cy={point.cy} r={point.r} fill={accentColor} stroke="currentColor" strokeWidth="1" className="text-primary/30" />
          ))}
        </g>
      ))}
